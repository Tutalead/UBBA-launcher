...
export default function ChangelogPage() {
  ...
  return (
    <div className="h-full flex flex-col gap-3">
      <h2 className="gothic uppercase text-sm tracking-[0.25em] text-bone-200">{t('changelog.title')}</h2>
          ...
          <div className="flex flex-col gap-4">
            {current.date && (
              <div className="flex items-center gap-2 text-[11px] gothic uppercase tracking-widest text-bone-400">
                <span>{t('changelog.released')}</span><span className="text-bone-200">{current.date}</span>
              </div>
            )}
            {current.pages && (
              <div className="flex items-center gap-2 text-[11px] gothic uppercase tracking-widest text-bone-400">
                <span>{t('changelog.pages')}</span><span className="text-bone-200">{current.pages}</span>
              </div>
            )}
            {current.url
              ? (
